...
class TimeMeasureScalaClassCreatorTADLRepetition extends TimeMeasureScalaClassCreator{
	...
	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int, eventCount: Int,
			lower: Int, upper: Int, span: Int, jitter: Int, printInputEvents: Boolean): Boolean= {
		var constraintCreator = new TimeMeasureScalaClassCreatorTADLRepetition
		try {
			...
			fileWriter.write("\n");
			constraintCreator.generateMemberVariableListExtended(fileWriter, 1, lower, upper, span, jitter)
			fileWriter.write("\n");
	...
	protected def generateMemberVariableListExtended(writer: FileWriter, prevTabCount: Int, lower: Int, upper: Int, span: Int,
			jitter: Int) : Unit = {
		writer.write(tabs(prevTabCount) + "var lower: Int = " + lower + "\n")
		writer.write(tabs(prevTabCount) + "var upper : Int = " + upper + "\n")
		writer.write(tabs(prevTabCount) + "var span : Int = " + span + "\n")
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount, lower, upper, span, jitter)" + "\n")
