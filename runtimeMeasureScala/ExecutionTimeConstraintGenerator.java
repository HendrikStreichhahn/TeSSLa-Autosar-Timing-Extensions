...
public class ExecutionTimeConstraintGenerator extends TraceSet{
	...
	public boolean generateTestTrace(int eventCount, int lower, int upper,
		...
		while (eventCount > 0) {
			...
			//start Event
			traces[0].insertEvent(new Event((int)timeNow, 0, traces[0])); 
			...
			for (int i = 0; i < numPreemptions; i++){
				...
				//preempt event
				traces[2].insertEvent(new Event((int)timeNow, 0, traces[2]));
				//resume event
				timeNow+= preemptLength;
				traces[3].insertEvent(new Event((int)timeNow, 0, traces[3]));
				eventCount-= 2;
			}
			//stop event
			timeNow+= segmentLength;
			traces[1].insertEvent(new Event((int)timeNow, 0, traces[1]));
			eventCount-= 2;
