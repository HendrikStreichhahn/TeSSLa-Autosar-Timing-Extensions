...
import java.io._
object measureTADL2StrongDelay{

	def main(args: Array[String]): Unit = {
		generateTrace()
		measureMultiple(5, "results/TADL2StrongDelay.txt")
	}
	...
	var lower: Int = 50
	var upper: Int = 50
	var sourceDistance: Int = 1
	var traceSet : StrongDelayConstraintGenerator = null
	...
	def measureConstraint() : (Long, Long, Long) = {
		...
		while (currentEvents.length != 0 && !(outputFinal && ! outputValue)) {
			for (i <- 0 to currentEvents.length-1){
				if (currentEvents(i).getOwnerStream().getName().equals("source"))
					tesslaMonitorInstance.set_var_source(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("target"))
					tesslaMonitorInstance.set_var_target(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
			}
	...
	def generateTrace(): Boolean = {
		tesslaMonitorInstance = new TesslaMonitor()
		traceSet = new StrongDelayConstraintGenerator()
		var res: Boolean = traceSet.generateTestTrace(eventCount, sourceDistance, sourceDistance, lower, upper)
		if (res) {
			traceSet.initOutput();
		}
		res
