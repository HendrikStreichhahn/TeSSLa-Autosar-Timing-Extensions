...
import java.io._
object measureTADL2Synchronization{
	def main(args: Array[String]): Unit = {
		generateTrace()
		measureMultiple(5, "results/TADL2Synchronization.txt")
	}
	...
	var maxClusterDistance : Int = 16
	var traceSet : SynchronizationConstraintGenerator = null
	...
	def measureConstraint() : (Long, Long, Long) = {
		...
		while (currentEvents.length != 0 && !(outputFinal && ! outputValue)) {
			for (i <- 0 to currentEvents.length-1){
				if (currentEvents(i).getOwnerStream().getName().equals("event 0" ))
					tesslaMonitorInstance.set_var_event0(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("event 1" ))
					tesslaMonitorInstance.set_var_event1(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("event 2" ))
					tesslaMonitorInstance.set_var_event2(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
			}
	...
	def generateTrace(): Boolean = {
		tesslaMonitorInstance = new TesslaMonitor()
		traceSet = new SynchronizationConstraintGenerator(streamCount)
		var res: Boolean = traceSet.generateTestTrace(eventCount,  tolerance, maxEventsPerClusterPerStream,minClusterDistance, maxClusterDistance)
		if (res) {
			traceSet.initOutput();
		}
		res
