...
import java.io._
object measureTADL2ExecutionTime{

	def main(args: Array[String]): Unit = {
		generateTrace()
		measureMultiple(5, "results/TADL2ExecutionTime.txt")
	}
	...
	var numPreemptions: Int = 1
	var preemptLength: Int = 100
	var traceSet : ExecutionTimeConstraintGenerator = null
	...
	def measureConstraint() : (Long, Long, Long) = {
		...
		while (currentEvents.length != 0 && !(outputFinal && ! outputValue)) {
			for (i <- 0 to currentEvents.length-1){
				if (currentEvents(i).getOwnerStream().getName().equals("start"))
					tesslaMonitorInstance.set_var_start(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("stop"))
					tesslaMonitorInstance.set_var_stop(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("preempt"))
					tesslaMonitorInstance.set_var_preempt(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("resume"))
					tesslaMonitorInstance.set_var_resume(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
	...
	def generateTrace(): Boolean = {
		tesslaMonitorInstance = new TesslaMonitor()
		traceSet = new ExecutionTimeConstraintGenerator()
		var res: Boolean = traceSet.generateTestTrace(eventCount, lower, upper, minDistance, maxDistance, numPreemptions, preemptLength)
		if (res) {
			traceSet.initOutput();
		}
		res
