...
import java.io._
object measureTADL2InputSynchronization{
	def main(args: Array[String]): Unit = {
		generateTrace()
		measureMultiple(5, "results/TADL2InputSynchronization.txt")
	}
	...
	var traceSet : InputSynchronizationConstraintGenerator = null
	...
	def measureConstraint() : (Long, Long, Long) = {
		...
		while (currentEvents.length != 0 && !(outputFinal && ! outputValue)) {
			for (i <- 0 to currentEvents.length-1){
				if (currentEvents(i).getOwnerStream().getName().equals("response " ))
					tesslaMonitorInstance.set_var_response(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("stimulus 1" ))
					tesslaMonitorInstance.set_var_stimulus1(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("stimulus 2" ))
					tesslaMonitorInstance.set_var_stimulus2(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("stimulus 3" ))
					tesslaMonitorInstance.set_var_stimulus3(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
			}
	...
	def generateTrace(): Boolean = {
		tesslaMonitorInstance = new TesslaMonitor()
		traceSet = new InputSynchronizationConstraintGenerator(streamCount)
