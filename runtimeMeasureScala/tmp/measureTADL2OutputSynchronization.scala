...
import java.io._
object measureTADL2OutputSynchronization{
	def main(args: Array[String]): Unit = {
		generateTrace()
		measureMultiple(5, "results/TADL2OutputSynchronization.txt")
	}

	var eventCount: Int = 100000
	...
	var tolerance: Int = 5
	var streamCount : Int = 3
	var maxEventsPerClusterPerStream : Int = 1
	var minClusterDistance : Int = 15
	var maxClusterDistance : Int = 15
	var traceSet : OutputSynchronizationConstraintGenerator = null
	...
	def measureConstraint() : (Long, Long, Long) = {
		...
		while (currentEvents.length != 0 && !(outputFinal && ! outputValue)) {
			for (i <- 0 to currentEvents.length-1){
				if (currentEvents(i).getOwnerStream().getName().equals("stimulus " ))
					tesslaMonitorInstance.set_var_stimulus(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("response 1" ))
					tesslaMonitorInstance.set_var_response1(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("response 2" ))
					tesslaMonitorInstance.set_var_response2(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
				if (currentEvents(i).getOwnerStream().getName().equals("response 3" ))
					tesslaMonitorInstance.set_var_response3(currentEvents(i).getColor(), currentEvents(i).getTimeStamp())
			}
	...
	def generateTrace(): Boolean = {
		tesslaMonitorInstance = new TesslaMonitor()
		traceSet = new OutputSynchronizationConstraintGenerator(streamCount)
		var res: Boolean = traceSet.generateTestTrace(eventCount, tolerance, minClusterDistance)
		if (res) {
			traceSet.initOutput();
		}
		res
