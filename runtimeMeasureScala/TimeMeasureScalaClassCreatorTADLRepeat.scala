...
class TimeMeasureScalaClassCreatorTADLRepeat extends TimeMeasureScalaClassCreator{
	...
	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int, eventCount: Int,
			lower: Int, upper: Int, span: Int, printInputEvents: Boolean): Boolean= {
		var constraintCreator = new TimeMeasureScalaClassCreatorTADLRepeat
		try {
			...
			fileWriter.write("\n");
			constraintCreator.generateMemberVariableListExtended(fileWriter, 1, lower, upper, span)
			fileWriter.write("\n");
	...
	protected def generateMemberVariableListExtended(writer: FileWriter, prevTabCount: Int, lower: Int, upper: Int, span: Int) : Unit = {
		...
		writer.write(tabs(prevTabCount) + "var traceSet : RepeatConstraintGenerator = null" + "\n")
	}
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "traceSet = new RepeatConstraintGenerator()" + "\n")
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount, lower, upper, span)" + "\n")
