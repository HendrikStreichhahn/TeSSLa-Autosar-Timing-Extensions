...
public class RepeatConstraintGenerator extends TraceSet{
	...
	public boolean generateTestTrace(int eventCount, int lower, int upper, int span){
		...
		for (int i = 0; i < span; i++){
			...
			traces[0].insertEvent(new Event(spanDistance * (i+1), 0, traces[0]));
			//System.out.println("" + spanDistance * (i+1) + ": event");
		}
		...
		for (int i = span; i < eventCount; i++){
			...
			boolean inserted = traces[0].insertEvent(new Event(nextEvent, 0, traces[0]));
