...
class TimeMeasureScalaClassCreatorARConcretePatternEventTriggering extends TimeMeasureScalaClassCreator{

	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int,
			eventCount: Int, offset: Array[Int], patternJitter: Int, patternLength: Int, patternPeriod: Int, printInputEvents: Boolean): Boolean= {
		var constraintCreator = new TimeMeasureScalaClassCreatorARConcretePatternEventTriggering
		try {
			...
			fileWriter.write("\n");
			constraintCreator.generateMemberVariableListExtended(fileWriter, 1, offset, patternJitter, patternLength,
				patternPeriod)
			fileWriter.write("\n");
	...
	protected def generateMemberVariableListExtended(writer: FileWriter, prevTabCount: Int, offset: Array[Int], patternJitter: Int,
			patternLength: Int, patternPeriod: Int) : Unit = {
		...
		writer.write(")\n")
		writer.write(tabs(prevTabCount) + "var patternJitter : Int = " + patternJitter + "\n")
		writer.write(tabs(prevTabCount) + "var patternLength : Int = " + patternLength + "\n")
		writer.write(tabs(prevTabCount) + "var patternPeriod : Int = " + patternPeriod + "\n")
		writer.write(tabs(prevTabCount) + "var traceSet : ARConcretePatternEventTriggeringGenerator = null" + "\n")
	}
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "traceSet = new ARConcretePatternEventTriggeringGenerator()" + "\n")
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount, patternPeriod, offset, patternJitter)" + "\n")
