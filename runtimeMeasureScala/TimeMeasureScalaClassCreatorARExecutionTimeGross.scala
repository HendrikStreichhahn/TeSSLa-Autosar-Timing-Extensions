...
class TimeMeasureScalaClassCreatorARExecutionTimeGross extends TimeMeasureScalaClassCreator{
	...
	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int, eventCount: Int,
		...
		var constraintCreator = new TimeMeasureScalaClassCreatorARExecutionTimeGross
		try {
			...
			fileWriter.write("\n")
			constraintCreator.generateMemberVariableListExtended(fileWriter, 1, lower, upper)
			fileWriter.write("\n")
	...
	protected def generateMemberVariableListExtended(writer: FileWriter, prevTabCount: Int, lower: Int, upper: Int) : Unit = {
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount, lower, upper, 1, 1, 0, 0)" + "\n")
