...
class TimeMeasureScalaClassCreatorARPeriodicEventTriggering extends TimeMeasureScalaClassCreator{

	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int,
			eventCount: Int, period: Int, jitter: Int, minimum: Int, printInputEvents: Boolean): Boolean= {
		var constraintCreator = new TimeMeasureScalaClassCreatorARPeriodicEventTriggering
		try {
			...
			fileWriter.write("\n");
			constraintCreator.generateMemberVariableListExtended(fileWriter, 1, period, jitter, minimum)
			fileWriter.write("\n");
	...
	protected def generateMemberVariableListExtended(writer: FileWriter, prevTabCount: Int, period: Int,
		...
		writer.write(tabs(prevTabCount) + "var traceSet : PeriodicConstraintGenerator = null" + "\n")
	}
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "traceSet = new PeriodicConstraintGenerator()" + "\n")
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount, period, jitter)" + "\n")
