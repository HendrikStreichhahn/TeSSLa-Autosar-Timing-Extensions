...
public class TimeMeasureARSynchronizationConstraintStimulus extends TimeMeasureConstraint{
	int streamCount, clusterDistance, tolerance;

	public TimeMeasureARSynchronizationConstraintStimulus(int streamCount, int clusterDistance, int tolerance){
	...
	public boolean generateTeSSLaFile(String fileName){
		try {
			...
			//output
			//fileWriter.write("def constraint :=  AUTOSAR_TIMEX.SynchronizationTimingConstraintStimulusSynchronization(eventsNow, " + streamCount +" ," + tolerance + ", false)\n");
			fileWriter.write("def constraint :=  AUTOSAR_TIMEX.SynchronizationTimingConstraintStimulusSynchronizationSingle(eventsNow, " + streamCount +" ," + tolerance + ")\n");
