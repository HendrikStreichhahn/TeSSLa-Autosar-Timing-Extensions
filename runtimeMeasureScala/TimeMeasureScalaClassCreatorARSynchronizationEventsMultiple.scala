...
class TimeMeasureScalaClassCreatorARSynchronizationEventsMultiple extends TimeMeasureScalaClassCreator{
	
	def generateEntireObject(scalaFilePath: String, scalaObjectName: String, resultFilePath: String, repetitionCount: Int,
			eventCount: Int, tolerance: Int, locStreamCount: Int, minClusterDistance: Int, maxClusterDistance: Int,
			maxEventsPerClusterPerStream: Int, printInputEvents: Boolean): Boolean= {
		var constraintCreator = new TimeMeasureScalaClassCreatorARSynchronizationEventsMultiple
		try {
	...
	protected def generateGenerateTrace(writer: FileWriter, prevTabCount: Int, repetitionCount: Int, resultFilePath: String) : Unit = {
		...
		writer.write(tabs(prevTabCount+1) + "var res: Boolean = traceSet.generateTestTrace(eventCount,  tolerance, maxEventsPerClusterPerStream," +
